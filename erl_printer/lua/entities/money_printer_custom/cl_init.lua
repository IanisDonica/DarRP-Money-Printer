...
function ENT:Draw()
    ...
    then
        ...
        draw.SimpleText("Money: $"..self:Geterl_printer_NMoney(),"DisplayFont2", 20,70, Color(252, 0, 0))
    ...
    then
        ...
        draw.SimpleText("Speed: "..self:Geterl_printer_MAET().."$/s", "DisplayFont2", 20,15,Color(255, 30, 0))
        draw.SimpleText("Max Storage: "..self:Geterl_printer_MaxStorage(), "DisplayFont2", 20,45,Color(255, 30, 0))
...
function ENT:Think()
    if self:Geterl_printer_MAET() < 256 then 
        self.upgrade_price = self:Geterl_printer_MAET()
    else
        self.upgrade_price = "max"
    end
    if self:Geterl_printer_MaxStorage() < 40000 then 
        self.max_storage = 10000
    else
        self.max_storage = "max"
    end
    if self.wasPressed then
        net.Start("erl_printer_money_extraction")
            net.WriteEntity(self.wasPressedwhere)
        net.SendToServer()
    end
    if self.wasPressed2 then
        net.Start("erl_printer_upgradeSpeed")
            net.WriteEntity(self.wasPressedwhere)
        net.SendToServer()
    end
    if self.wasPressed3 then
        net.Start("erl_printer_upgradeStorage")
            net.WriteEntity(self.wasPressedwhere)
        net.SendToServer()
    end
end
